...
class PlayersCar extends Car {
    ...
    CollideWithPedestrian(pedestrians) {
        let hit = false;
        if (this.y < pedestrians.spriteY + pedestrians.SCALED_HEIGHT && this.y + this.image.height * scale > pedestrians.spriteY) {
            if (this.x < pedestrians.spriteX + pedestrians.SCALED_WIDTH && this.x + this.image.width * scale > pedestrians.spriteX) {
                hit = true;
            }
        }
        return hit;
...
class Pedestrian extends Sprite {
    ...
    PedestrianKill(pedestrian) {
        for (let i = 0; i < pedestrians.length; i++) {
            if (pedestrians[i].spriteX + pedestrians[i].SCALED_WIDTH >= 1080) {
...
function Draw0() { //Кнопка старт
    ...
    start.onload = function() {
        if (!start.complete) {
            console.log("Start image is not yet loaded");
            return;
        }
...
window.onload = function() {
    Resize(); // При загрузке страницы задаётся размер холста
    try {
        Draw0(); // Отрисовка кнопки "Старт"
    } catch (error) {
        console.error("Error during Draw0 function:", error);
    }
}; 